...
/**
 * Generates an RTF output stream (java.io.OutputStream) from rich text (handed off through a series of LTTextAcceptor
 * calls). Can be used to generate RTF from any object which knows how to write to a text acceptor (e.g.,
 * LTAttributedText and LTRTFFilter).
 * 
 * <p>
...
class RTFGenerator extends Object {
	...
	/*
	 * An array of character-keyword pairs. This could be done as a dictionary (and lookup would be quicker), but that
	 * would require allocating an object for every character written (slow!).
	 */
	...
	public void writeRTFHeader() throws IOException {
		...
		/*
		 * TODO: Should the writer attempt to examine the text it's writing and pick a character set which will most
		 * compactly represent the document? (currently the writer always uses the ansi character set, which is roughly
		 * ISO-8859 Latin-1, and uses Unicode escapes for all other characters. However Unicode is a relatively recent
		 * addition to RTF, and not all readers will understand it.)
		 */
		writeBegingroup();
	...
	void updateParagraphAttributes(MutableAttributeSet current, AttributeSet newAttributes, boolean emitStyleChanges) throws IOException {
		...
		/*
		 * The only way to get rid of tabs or styles is with the \pard keyword, emitted by resetParagraphAttributes().
		 * Ideally we should avoid emitting \pard if the new paragraph's tabs are a superset of the old paragraph's
		 * tabs.
		 */
	 ...
	 * 
	 * buf = "["; for(i = 0; i < ts.length; i++) { buf = buf + ts[i].toString(); if ((i+1) < ts.length) buf = buf + ",";
	 * } return buf + "]"; }
	 */
	...
	void updateCharacterAttributes(MutableAttributeSet current, AttributeSet newAttributes, boolean updateStyleChanges) throws IOException {
		...
		checkNumericControlWord(current, newAttributes, StyleConstants.LineSpacing, "sl", 0, 20f); /*
																									 * TODO: sl
																									 * wackiness
																									 */
	...
	public void writeCharacter(char ch) throws IOException {
		...
		if (b == 0) {
			...
			/*
			 * In some cases it would be reasonable to check to see if the glyph being written out is in the Symbol
			 * encoding, and if so, to switch to the Symbol font for this character. TODO.
			 */
			/*
	...
	/**
	 * Takes a translation table (a 256-element array of characters) and creates an output conversion table for use by
	 * convertCharacter().
	 */
	/*
	 * Not very efficient at all. Could be changed to sort the table for binary search. TODO. (Even though this is
	 * inefficient however, writing RTF is still much faster than reading it.)
	 */
	...
	/**
	 * Takes a char and a conversion table (an int[] in the current implementation, but conversion tables should be
	 * treated as an opaque type) and returns the corresponding byte value (as an int, since bytes are signed).
	 */
