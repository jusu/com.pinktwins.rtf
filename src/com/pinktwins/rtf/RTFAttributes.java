...
class RTFAttributes {
	...
	static {
		...
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabAlignmentKey, "tqr", TabStop.ALIGN_RIGHT));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabAlignmentKey, "tqc", TabStop.ALIGN_CENTER));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabAlignmentKey, "tqdec", TabStop.ALIGN_DECIMAL));

		a.addElement(new AssertiveAttribute(PST, RTFReader.TabLeaderKey, "tldot", TabStop.LEAD_DOTS));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabLeaderKey, "tlhyph", TabStop.LEAD_HYPHENS));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabLeaderKey, "tlul", TabStop.LEAD_UNDERLINE));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabLeaderKey, "tlth", TabStop.LEAD_THICKLINE));
		a.addElement(new AssertiveAttribute(PST, RTFReader.TabLeaderKey, "tleq", TabStop.LEAD_EQUALS));
	...
	static class NumericAttribute extends GenericAttribute implements RTFAttribute {
		...
		public boolean writeValue(Object o_value, RTFGenerator target, boolean force) throws IOException {
			...
			if (value == null) {
				/*
				 * TODO: What is the proper behavior if the Swing object does not specify a value, and we don't know its
				 * default value? Currently we pretend that the RTF default value is equivalent (probably a workable
				 * assumption)
				 */
				return true;
			}
