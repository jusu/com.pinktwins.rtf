 ...
 * occasional ``specials'' (formatting characters).
 * 
 * <p>
 * Most of the functionality of this class would be redundant except that the <code>ByteToChar</code> converters are
 * suddenly private API. Presumably this class will disappear when the API is made public again. (sigh) That will also
 * let us handle multibyte character sets...
 * 
 * <P>
 * A subclass should override at least <code>write(char)</code> and <code>writeSpecial(int)</code>. For efficiency's
 * sake it's a good idea to override <code>write(String)</code> as well. The subclass' initializer may also install
 * appropriate translation and specials tables.
 * 
 * @see OutputStream
 */
abstract class AbstractFilter extends OutputStream {
	...
	/**
	 * A table indicating which byte values should be interpreted as characters and which should be treated as
	 * formatting codes
	 */
	...
	/**
	 * A convenience method that reads text from a FileInputStream and writes it to the receiver. The format in which
	 * the file is read is determined by the concrete subclass of AbstractFilter to which this method is sent.
	 * <p>
	 * This method does not close the receiver after reaching EOF on the input stream. The user must call
	 * <code>close()</code> to ensure that all data are processed.
	 * 
	 * @param in
	 ...
	 * 
	 * <p>
	 * <strong>PENDING:</strong> Does <code>write(byte[])</code> call <code>write(byte[], int, int)</code> or is it the
	 * other way around?
	 */
	...
	/**
	 * Subclasses must provide an implementation of this method which accepts a single special byte. No translation is
	 * performed on specials.
	 * 
	 * @param b
